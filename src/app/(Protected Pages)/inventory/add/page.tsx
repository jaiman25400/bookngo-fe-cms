...
export default function AddInventoryPage() {
  const [formData, setFormData] = useState({
    ...
    sizes: [{ size: "", quantity: 0 }], // Changed from null to 0
  });
  ...
  const handleSizeChange = (
    ...
    updatedSizes[index] = {
      ...updatedSizes[index],
      [name]: value, // Update the size or quantity based on the name
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const newInventory = await addInventory(formData);
      ...
      setMessage({ type: "error", text: error.message || "Error adding inventory" });
    } finally {
      setLoading(false);
    }
  };
